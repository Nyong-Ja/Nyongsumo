...
(() => {
    ...
    const makeSignature = (g, s) => {
        if (!s) return '';
        return `<section class="ghost-signature-hero"><div class="ghost-signature-kicker">${s.kicker}</div><h2>${s.headline.replace(/\n/g,'<br>')}</h2><p>${s.desc}</p><div class="ghost-signature-stats">${s.cards.map((c,i)=>`<div class="ghost-signature-stat ghost-signature-stat-${i+1}"><span>${c[0]}</span><strong>${c[1]}</strong><small>${c[2]}</small></div>`).join('')}</div></section>`;
    };
    const make = (g, old, x) => {
