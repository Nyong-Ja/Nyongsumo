...
function renderGhostList() {
    ...
    requestAnimationFrame(() => {
        container.querySelectorAll('.ghost-signature-desc').forEach(desc => {
            const overflow = desc.scrollWidth - desc.clientWidth;
            if (overflow > 2) {
                desc.classList.add('is-overflow');
                desc.style.setProperty('--signature-shift', `${overflow}px`);
